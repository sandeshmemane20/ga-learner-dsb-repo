...
# Merge dataframes
df = transactions.merge(offers,how="left",on="Offer #")
...
data = clusters.merge(transactions,how='left',on='Customer Last Name')

# merge `data` and `offers`
data = data.merge(offers,how='left',on='Offer #')
...
for i in range(0,5):
    ...
    s = new_df['Discount (%)'].sum()
    n = len(new_df['Discount (%)'])
    counts = s/n
